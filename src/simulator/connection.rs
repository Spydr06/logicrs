use crate::{renderer::{*, vector::*}, id::Id, application::editor::EditorMode};
use super::*;
...
impl Segment {
    ...
    fn render<R>(&self, active: bool, start: Vector2<i32>, renderer: &R, plot: &Plot) -> Result<(), R::Error>
        where R: Renderer
    {
        match self {
    ...
    fn get_segment_mut(&mut self, location: &SegmentLocation, depth: &mut usize) -> Option<&mut Segment> {
...
impl Connection {
    ...
    pub fn new_basic(origin_block: BlockID, origin_port: u8, destination_block: BlockID, destination_port: u8) -> Self {
        Self {
            id: Id::new(),
            active: false,
...
fn render_waypoint<R>(position: Vector2<i32>, active: bool, highlighted: bool, renderer: &R) -> Result<(), R::Error>
    where R: Renderer
{
    let connector_color = unsafe { if active { &COLOR_THEME.enabled_fg_color } else { &COLOR_THEME.disabled_fg_color } };
    renderer
        .set_line_width(1.)
...
fn render_block_connector<R>(position: Vector2<i32>, active: bool, highlighted: bool, renderer: &R) -> Result<(), R::Error>
    where R: Renderer
{
    let connector_color = unsafe { if active { &COLOR_THEME.enabled_fg_color } else { &COLOR_THEME.disabled_fg_color } };
    renderer
...
fn render_line<R>(active: bool, start: Vector2<i32>, end: Vector2<i32>, renderer: &R) -> Result<(), R::Error>
    where R: Renderer
{
    renderer.set_color(unsafe { if active { &COLOR_THEME.enabled_bg_color } else { &COLOR_THEME.disabled_bg_color } })
        .set_line_width(4.);
...
impl Renderable for Connection {
        ...
        where R: Renderer
    {
        ...
        render_block_connector(origin_pos, self.active, origin_block.highlighted(), renderer)
    }
}
